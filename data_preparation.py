...
def prepare_data_in_dir(input_dir, output_dir, rep_length, overlapping_reads, log, cpu_count, max_memory):
    ...
    if overlapping_reads == "Y" or overlapping_reads == "P":
        if len(files) == 2:
            ...
            raise Exception(f"when parameter overlapping_reads is set to {overlapping_reads}, there should be 2 fastq "
                            f"files in input_dir but found only one! Consult docs for details.")
    else:
        for file in files:
            if file_type == 'gz':
